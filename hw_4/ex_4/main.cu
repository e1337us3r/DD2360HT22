...
int main(int argc, char **argv)
{
  ...
  gpuCheck(cudaMallocManaged(&ARowPtr, sizeof(int) * rowSize));
  ...
  if (concurrentAccessQ)
  {
    ...
    gpuCheck(cudaMemPrefetchAsync(ARowPtr, sizeof(int) * rowSize, cudaCpuDeviceId));
  ...
  if (concurrentAccessQ)
  {
    ...
    gpuCheck(cudaMemPrefetchAsync(ARowPtr, sizeof(int) * rowSize, device));
  ...
  //@@ Insert code to set the cuBLAS  and cuSPARSE pointer mode to CUSPARSE_POINTER_MODE_HOST
  ...
  //@@ vector (SpMV) CSR routine of cuSPARSE'
  // create vector descriptors for both sides
  cusparseDnVecDescr_t vecX;
  cusparseDnVecDescr_t vecY;
  cusparseSpMVAlg_t algorithm = CUSPARSE_SPMV_CSR_ALG1;
  ...
  cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, operation, &one, Adescriptor, vecX, &zero, vecY, CUDA_R_64F, algorithm, &bufferSize));
  ...
  for (int it = 0; it < nsteps; ++it)
  {
    ...
    //@@ tmp = 1 * A * temp + 0 * tmp
    cusparseCheck(cusparseSpMV(cusparseHandle, operation, &one, Adescriptor, vecX, &zero, vecY, CUDA_R_64F, algorithm, buffer));
  ...
  cublasCheck(cublasDaxpy(cublasHandle, dimX, &negative_one, temp, 1, tmp, 1));
